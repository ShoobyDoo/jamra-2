...
export const loadAppConfig = (): AppConfig => {
  ...
  // Use RESOURCES_PATH if available (Tauri/Electron), otherwise use cwd
  const resourcesPath = process.env.RESOURCES_PATH ?? process.cwd();

  const defaultInstallDir =
    process.env.EXTENSIONS_INSTALL_DIR ??
    path.join(resourcesPath, "resources", "extensions");

  const defaultCatalogPath =
    process.env.CATALOG_REPO_URL ??
    path.join(resourcesPath, "resources", "catalog", "default", "index.json");
  ...
  return {
    ...
    extensions: {
      ...
      bundleDir:
        process.env.EXTENSIONS_BUNDLE_DIR ??
        path.join(resourcesPath, "resources", "extensions", "bundles"),
      runtime: {
    ...
    installer: {
      ...
      tempDir:
        process.env.INSTALLER_TEMP_DIR ??
        path.join(resourcesPath, "temp", "extensions"),
