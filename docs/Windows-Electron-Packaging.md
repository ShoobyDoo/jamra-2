...
This is the working recipe that produces a fast, functional Windows installer and a portable EXE for `JAMRA`.
...
- `electron/main.ts`
  ...
  - Minimal file logger (no dialogs), logs to `%APPDATA%/JAMRA/logs/main.log`
...
- Package for Windows (installer + portable):
  - `pnpm make`
- Outputs: `release/JAMRA-<version>-Setup.exe` and `release/JAMRA <version>.exe`

## Logging & Diagnostics

- File log only (no dialogs): `%APPDATA%/JAMRA/logs/main.log`
